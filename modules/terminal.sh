...
link_wezterm_config() {
    ...
    if is_macos; then
        ...
        # For WSL, we link to WSL home - Windows config is separate
        safe_symlink "$config_dir/.wezterm.lua" "$HOME/.wezterm.lua"

        # Also try to link to Windows user directory if accessible
        local win_home
        ...
        if [[ -n "$win_home" && -d "$win_home" ]]; then
            ...
            # Note: symlinks from Windows to WSL don't work well
            # Instead, we can copy or suggest manual setup
            if [[ -f "$win_home/.wezterm.lua" ]]; then
                log_info "WezTerm config exists in Windows home"
                log_info "For WSL integration, ensure Windows config sources WSL config or use the same file"
            fi
        fi
    else
        safe_symlink "$config_dir/.wezterm.lua" "$HOME/.wezterm.lua"
